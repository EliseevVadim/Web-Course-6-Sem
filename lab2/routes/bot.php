...
ShopServiceFacade::bot()
    ->addRoute("/start", function ($message) {
        ...
        return [
            "text" => "<b>Доброго времени суток!</b><br>Вас приветствует бот-помощник автосервиса N. Для дальнейшей работы выберите один из предложенных ниже разделов:",
            "keyboard" => [
                ["text" => "Категории услуг ($categories)"],
                ["text" => "Справка"],
                ["text" => "Заказы ($ordersNum)"],
                ShopServiceFacade::bot()->currentUser()->role_id == 2 ? ["text" => "Только для модераторов"] : []
            ]
        ];
    }, "start");
